def createTestSet(ListOfFeatureObject, ListOfData, ListOfLabel):
    #Create test set
    Y_test_1 = []
    X_test_1 = []

    i = 0
    for featureObj in ListOfFeatureObject:
        Y_test_1 += ListOfLabel[i] * len(featureObj[ListOfData[0]])
        X_test_1 += np.array([featureObj[key] for key in ListOfData]).T
